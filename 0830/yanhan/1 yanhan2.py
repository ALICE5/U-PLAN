...
urls = ["http://www.mayi.com/beijing/{}/".format(i) for i in range(1,3)]
headers = {
...
def crawler(urls,headers):
    ...
    for url in urls:
        ...
        images = Soup.select("#searchRoom > dd > div > div:nth-of-type(2) > div > a:nth-of-type(1) ")

        for title, price, image in zip(titles, prices, images):
            data = {
                "title": title.get_text(),
                "price": int(price.get_text().strip().split(" ")[0].strip()[1:]),
                "image": "http:" + image.get("mainimage")
            }
            yanhan.insert_one(data)
